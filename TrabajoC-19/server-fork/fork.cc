...
void udp_listener(const std::string& listen_ip) {
    ...
    if (sockfd < 0) {
        atomic_cout("\n[Listener] Error en socket para " + listen_ip + ": " + strerror(errno) + "\n");
        return;
    }
    ...
    inet_pton(AF_INET, listen_ip.c_str(), &servaddr.sin_addr);
    servaddr.sin_port = htons(MULTICAST_PORT);

    if (bind(sockfd, (struct sockaddr*)&servaddr, sizeof(servaddr)) < 0) {
        atomic_cout("[Listener] Bind falló en " + listen_ip + ": " + strerror(errno) + "\n");
        close(sockfd);
        return;
    }
    ...
    while (running) {
        ...
        if (n > 0) {
            ...
            // Parsear respuesta del servidor: "Nombre|IP|fig1,fig2"
            size_t p1 = msg.find('|');
