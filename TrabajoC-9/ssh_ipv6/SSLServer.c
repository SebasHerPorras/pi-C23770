...
SSL_CTX* InitServerCTX( void ) {
   ...
   SSL_load_error_strings();   /* load all error messages. Can debug using ERR_get_error()*/
...
\t<Description>Redes y sistemas operativos</Description>\n\
...
\t<Password>ci0123</Password>\n\
   ...
   // SSL accept se encarga de la negociacion del protocolo ssl del nuevo socket creado, el cual
   // esta asociado a la estructura SSL
...
int main( int cuantos, char ** argumentos ) {
   ...
   while (1) {
      ...
      SSL * ssl;
      int client = accept( server /*fd socket server*/, (struct sockaddr*) &addr, &len );  /* accept connection as usual */
